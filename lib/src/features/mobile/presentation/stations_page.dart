      ...
      child: stationsAsync.when(
        ...
        error: (e, _) => ListView(
          children: [
            ...
            Center(child: Text('Error estaciones: $e')),
          ],
        ),
        data: (locations) {
            ...
            itemBuilder: (_, i) {
              ...
              return _LocationCard(
                ...
                onStart: (stationId, chargeBoxId, connectorId) async {
                  ...
                  try {
                    final res = await widget.api.startStation(stationId, connectorId: int.tryParse(connectorId));
                    messenger.showSnackBar(
                      SnackBar(content: Text(res['message']?.toString() ?? 'Iniciando carga...')),
                    );
                ...
                onStop: (stationId) async {
                  ...
                  try {
                    ...
                    messenger.showSnackBar(
                      SnackBar(content: Text(res['message']?.toString() ?? 'Deteniendo carga...')),
                    );
                  } on DioException catch (e) {
                     final errorMsg = e.response?.data?['message'] ?? e.message ?? e.toString();
                     messenger.showSnackBar(SnackBar(content: Text(errorMsg), backgroundColor: Colors.red));
                ...
                return Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Icon(Icons.ev_station, size: 20, color: isThisStationActive ? const Color(0xFF0076D6) : const Color(0xFF444444)),
                          const SizedBox(width: 8),
                          Expanded(
                            child: Text(
                              station['name']?.toString() ?? 'Cargador',
                              style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: isThisStationActive ? const Color(0xFF0076D6) : const Color(0xFF333333)),
                            ),
                          ),
                          Text('ID: $chargeBoxId', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey.shade500)),
                        ],
                      ),
                          ...
                          return GestureDetector(
                            ...
                            child: AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(color: isSelected ? const Color(0xFF0076D6) : color.withOpacity(0.4), width: 2),
                                boxShadow: isSelected ? [BoxShadow(color: Colors.blue.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 2))] : null,
                              ),
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    ...
                                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: isSelected ? Colors.white : color),
                                  ),
                                ],
                              ),
                            ),
                        ...
                        children: [
                          Expanded(
                            child: ElevatedButton.icon(
                              icon: isProcessing ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.play_arrow, size: 18),
                              ...
                              onPressed: (isProcessing || isGlobalLocked || selectedConnectorId == null || !selectedConnectorId!.startsWith(chargeBoxId))
                                  ? null
                          ...
                          Expanded(
                            child: OutlinedButton.icon(
                              ...
                              style: OutlinedButton.styleFrom(
                                ...
                                padding: const EdgeInsets.symmetric(vertical: 12),
                                side: BorderSide(color: isThisStationActive ? Colors.red : const Color(0xFF0076D6), width: 1.5),
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                              ),
                      ...
                      if (isGlobalLocked && !isThisStationActive)
                        const Padding(
                          ...
                          child: Text('Hay otra carga activa. Debes detenerla primero.', style: TextStyle(fontSize: 10, color: Colors.orange, fontWeight: FontWeight.bold)),
                        ),
                    ],
                  ),
                );
