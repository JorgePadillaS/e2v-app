...
class _QrConnectorSelectPageState extends State<QrConnectorSelectPage> {
  ...
  String _statusRaw(Map<String, dynamic> c) =>
      (c['status'] ?? '').toString().toUpperCase();

  bool _isError(String status) =>
  ...
  Widget build(BuildContext context) {
    ...
    Widget hoseCard(Map<String, dynamic> c, String forcedLabel) {
      ...
      final selected =
          connectorId != null && connectorId == _selectedConnectorId;
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isConfirmationMode ? 'Confirmar Carga' : 'Seleccionar manguera'),
      ),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            if (!isConfirmationMode) ...[
              ...
              const Spacer(),
              const Text('Leyenda: gris parpadeante = disponible · amarillo = ocupada · rojo = error · verde = seleccionada', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: Colors.grey)),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      _selectedConnectorId == 1 ? 'CCS2' : (_selectedConnectorId == 2 ? 'GBT' : 'CONECTOR $_selectedConnectorId'),
