...
class _PaymentWebViewModalState extends State<PaymentWebViewModal> {
  ...
  Future<void> _saveBytes(Uint8List bytes, String fileName, String ext) async {
    ...
    await FileSaver.instance.saveFile(name: name, bytes: bytes, fileExtension: ext, mimeType: _mimeFromExtension(ext));
  }
  ...
  Future<void> _downloadToDownloads(String url) async {
    ...
    try {
      ...
      final response = await Dio(
        BaseOptions(connectTimeout: const Duration(seconds: 8), receiveTimeout: const Duration(seconds: 12)),
      ).get<List<int>>(
        url,
        options: Options(
      ...
      if (src.startsWith('data:image/')) {
        ...
        try {
          ...
          final ext =
              meta.contains('png')
                  ? 'png'
                  : (meta.contains('jpeg') || meta.contains('jpg'))
                  ? 'jpg'
                  : 'img';
      ...
      child: SizedBox(
        ...
        child: Column(
          children: [
            ...
            if (_downloading)
              const Padding(padding: EdgeInsets.fromLTRB(10, 8, 10, 4), child: LinearProgressIndicator(minHeight: 3)),
